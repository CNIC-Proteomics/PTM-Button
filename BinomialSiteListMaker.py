...
if __name__ == '__main__':
    ...
    if args.config:
        ...
        config.optionxform = str  # Mantener las claves en mayúsculas/minúsculas originales
        config.read(args.config)

        params = {}
        
        for key, value in config.items('BinomialSiteListMaker_Parameters'):
            if value is not None:  # Evitar valores nulos
                value = value.split("#")[0].strip()  # Eliminar comentarios y espacios extra
            params[key.strip()] = value

        # Convertir valores específicos
        if 'x' in params:
            params['x'] = int(params['x'])  # Convertir 'x' a entero
        if 'show_unassigned' in params:
            params['show_unassigned'] = params['show_unassigned'].lower() == 'true'  # Convertir a booleano
