...
def main(file,infile1, infilefasta):
    """
    ...
    # Initialize an empty dictionary to store modifications with new names and masses
    # Iterate through each fixed modification option and value from the config
    dic_mod = {}
    for option, value in config["Fixed Modifications"].items():
        option = option.upper()
        # split the value to separate the mass and the label, if it exists
        parts = value.split("\t")
        mass_value = float(parts[0].strip().replace(",", "."))  # parse mass value
        new_name = parts[1][1:] if len(parts) > 1 else ""  # extract label if it exists
        # if option is NT, set NT label mass if it hasn't been set yet
        if option == "NT":
            NT_label = mass_value
        else:
            # set the selected amino acid if mass matches NT label
            if mass_value == NT_label:
                selectedaa = option
            # calculate total value including base mass of amino acid
            value2 = mass_value + dic_aa.get(option, 0)
            dic_mod[option[0]] = (new_name, value2, mass_value)
...
if __name__ == '__main__':
    ...
    main(args.config, infile1,infilefasta)
